...
async function printItems() {
    let response = await axios.get("/api/getprocurementfaqs");
    data.value = response.data;
}

async function save() {
    if (editedIndex.value === -1) {
        await axios.post("/api/createprocurementfaqs", editedItem.value);
        printItems();
        this.close()
        // EditSnackbar.value = true
    } else if (editedIndex.value >= 0) {
        await axios.post("/api/updateprocurementfaqs", editedItem.value);
        dialog.value = false
        printItems();
        // EditSnackbar.value = true
    }
...
async function deleteItemConfirm(item) {
    await axios.post('/api/deleteprocurementfaqs', item);
