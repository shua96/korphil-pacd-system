...
async function printItems() {
    let response = await axios.get("/api/getaccountingfaqs");
    data.value = response.data;
}

async function save() {
    if (editedIndex.value === -1) {
        await axios.post("/api/createaccountingfaqs", editedItem.value);
        printItems();
        this.close()
        // EditSnackbar.value = true
    } else if (editedIndex.value >= 0) {
        await axios.post("/api/updateaccountingfaqs", editedItem.value);
        dialog.value = false
        printItems();
        // EditSnackbar.value = true
    }
...
async function deleteItemConfirm(item) {
    await axios.post('/api/deleteaccountingfaqs', item);
