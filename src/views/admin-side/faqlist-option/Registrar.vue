...
async function printItems() {
    let response = await axios.get("/api/getregistrarfaqs");
    data.value = response.data;
}

async function save() {
    if (editedIndex.value === -1) {
        await axios.post("/api/createregistrarfaqs", editedItem.value);
        printItems();
        this.close()
        // EditSnackbar.value = true
    } else if (editedIndex.value >= 0) {
        await axios.post("/api/updateregistrarfaqs", editedItem.value);
        dialog.value = false
        printItems();
        // EditSnackbar.value = true
    }
...
async function deleteItemConfirm(item) {
    await axios.post('/api/deleteregistrarfaqs', item);
